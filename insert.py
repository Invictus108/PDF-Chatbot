        ...
        payload = {
            ...
            "messages": [
                ...
                "content": [
                    {
                    "type": "text",
                    "text": f"Use this context: {text.replace('\n', '<br>') if text else 'No context in this query'}, and these tables {'<br>'.join([table.to_string(index=False) for table in tables]).replace('\n', '<br>') if tables else 'No tables in this query'} alongside the images to answer this question {question}"
                    },
